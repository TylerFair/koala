...
void OrbitDerivatives::compute_orbit_and_derivatives(
  const double time, double& out_d, double& out_z, double& out_nu,
  double out_dd_dz[], double out_dnu_dz[]) {
  ...
  if (m_ecc == 0.) {
    this->compute_circular_orbit_and_derivatives(
      time, out_d, out_z, out_nu, out_dd_dz, out_dnu_dz);
  } else {
    this->compute_eccentric_orbit_and_derivatives(
      time, out_d, out_z, out_nu, out_dd_dz, out_dnu_dz);
  }
}
