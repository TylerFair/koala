...
/**
 * JAX custom XLA call. Transit light curve computation and derivatives
 * for a power-2 limb-darkened star. The derivatives computed are df/dz
 * where z={t0, p, a, i, e, w, c, alpha, {rs}}.
 *
 * @param out_tuple tuple(f, df_dz) where f is an array allocated for
 *        the light curve flux values, and df_dz is a ravelled array
 *        allocated for the flux derivatives. This pre-ravelled array
 *        has dimensions [..., 6 + n_us + n_rs] where we have 6 orbital,
 *        n_us=2 limb-darkening, and n_rs transmission string parameters.
 * @param in list(t0, p, a, i, e, w, c, alpha, r0, r1, r2,..).
 * @return void.
 */
...
/**
 * JAX custom XLA call. Flux-only transit light curve computation for a
 * power-2 limb-darkened star. This avoids materializing df/dz when only
 * primal flux values are required.
 *
 * @param out array allocated for the light curve flux values.
 * @param in list(t0, p, a, i, e, w, c, alpha, r0, r1, r2,..).
 * @return void.
 */
...
/**
 * Optional JAX custom-call registrations for CUDA-enabled builds.
 *
 * The returned dictionary is empty when the extension was built without
 * HARMONICA_ENABLE_CUDA.
 */
