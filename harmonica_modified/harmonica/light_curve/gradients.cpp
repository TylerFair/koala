...
FluxDerivatives::FluxDerivatives(
  ...
  if (m_ld_law == limb_darkening::quadratic) {
  ...
  } else if (m_ld_law == limb_darkening::non_linear) {
    ...
    double a = m_power2_alpha + 2.;
    m_dI0_dc = m_power2_alpha / (fractions::pi * a * m_I_0_bts);
    m_dI0_dpower2_alpha =
      2. * m_power2_c / (fractions::pi * a * a * m_I_0_bts);

    m_dalpha_ds0 = m_I_0 * (1. - m_power2_c);
    m_dalpha_dsalpha = m_I_0 * m_power2_c;
  }
...
void FluxDerivatives::transit_flux_and_derivatives(
  ...
  if (m_ld_law == limb_darkening::quadratic) {
    ...
    m_alpha = m_I_0 * (
      (1. - m_power2_c) * m_s0 + m_power2_c * m_salpha
    );
  }
  out_f = 1. - m_alpha;
...
void FluxDerivatives::s_star(int _j, int theta_type_j, double _theta_j,
  ...
  std::vector<std::complex<double>> dsn_phi_j_rp_dcs(m_n_rs, 0.);
  std::vector<std::complex<double>> dsn_phi_j_p1_rp_dcs(m_n_rs, 0.);
  ...
  if (m_ld_law == limb_darkening::quadratic) {
    ...
    m_s0 += fractions::one_half * phi_diff;
    m_salpha += phi_diff / (m_power2_alpha + 2.);
  }
  ...
  std::vector<std::complex<double>> dsn_dcs(m_n_rs, 0.);
  for (int n = -m_N_c; n < m_N_c + 1; n++) {
    int npN_c = n + m_N_c;
  ...
  if (m_ld_law == limb_darkening::quadratic) {
    ...
    double salpha_factor = 1. / (m_power2_alpha + 2.);
    m_ds0_dd += fractions::one_half * dsn_dd;
    m_dsalpha_dd += salpha_factor * dsn_dd;

    m_ds0_dnu += fractions::one_half * dsn_dnu;
    m_dsalpha_dnu += salpha_factor * dsn_dnu;
    m_dsalpha_dpower2_alpha += -phi_diff / (
      (m_power2_alpha + 2.) * (m_power2_alpha + 2.)
    );

    for (int n = -m_N_c; n < m_N_c + 1; n++) {
      int npN_c = n + m_N_c;
      m_ds0_dcs(npN_c) += fractions::one_half * dsn_dcs[npN_c];
      m_dsalpha_dcs(npN_c) += salpha_factor * dsn_dcs[npN_c];
    }
  }
}
...
void FluxDerivatives::f_derivatives(double df_dz[]) {
  ...
  if (m_ld_law == limb_darkening::quadratic) {
    ...
    df_dz[0] = m_df_dalpha * (
      m_dalpha_ds0 * m_ds0_dd + m_dalpha_dsalpha * m_dsalpha_dd
    );
    df_dz[1] = m_df_dalpha * (
      m_dalpha_ds0 * m_ds0_dnu + m_dalpha_dsalpha * m_dsalpha_dnu
    );

    double dalpha_dc = m_I_0 * (m_salpha - m_s0);
    double dalpha_dpower2_alpha = m_I_0 * m_power2_c * m_dsalpha_dpower2_alpha;
    df_dz[2] = m_df_dalpha * (dalpha_dI0 * m_dI0_dc + dalpha_dc);
    df_dz[3] = m_df_dalpha * (
      dalpha_dI0 * m_dI0_dpower2_alpha + dalpha_dpower2_alpha
    );

    m_df_dcs = m_df_dalpha * (
      m_dalpha_ds0 * m_ds0_dcs + m_dalpha_dsalpha * m_dsalpha_dcs
    );
    for (int n = 0; n < m_N_c + 1; n++) {
      if (n == 0) {
        df_dz[4] = (m_df_dcs(m_N_c) * m_dc0_da0).real();
...
void FluxDerivatives::reset_derivatives() {
  ...
  m_ds0_dd = 0., m_ds12_dd = 0., m_ds1_dd = 0., m_ds32_dd = 0.,
  m_ds2_dd = 0., m_dsalpha_dd = 0.;
  m_ds0_dnu = 0., m_ds12_dnu = 0., m_ds1_dnu = 0., m_ds32_dnu = 0.,
  m_ds2_dnu = 0., m_dsalpha_dnu = 0., m_dsalpha_dpower2_alpha = 0.;

  m_ds0_dcs.setZero(), m_ds12_dcs.setZero(), m_ds1_dcs.setZero(),
  m_ds32_dcs.setZero(), m_ds2_dcs.setZero(), m_dsalpha_dcs.setZero();
}
...
void FluxDerivatives::analytic_even_terms(
  ...
  std::vector<std::complex<double>> ds0_q0_dcs(m_n_rs, 0.);
  std::vector<std::complex<double>> ds2_q2_dcs(m_n_rs, 0.);
...
void FluxDerivatives::numerical_odd_terms(
  ...
  std::vector<std::complex<double>> ds1_zeta_zp_rp_dcs(m_n_rs, 0.);
  std::vector<std::complex<double>> ds1_eta_rp_dcs(m_n_rs, 0.);
  std::vector<std::complex<double>> ds1_eta_rpdash_dcs(m_n_rs, 0.);
  ...
  if (m_ld_law == limb_darkening::quadratic) {
    ...
    std::vector<std::complex<double>> ds12_zeta_zp_rp_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> ds12_eta_rp_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> ds12_eta_rpdash_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> ds1_zeta_zp_rp_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> ds1_eta_rp_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> ds1_eta_rpdash_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> ds32_zeta_zp_rp_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> ds32_eta_rp_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> ds32_eta_rpdash_dcs(m_n_rs, 0.);
    ...
    double salpha_planet = 0.;

    double dsalpha_zeta_zp_dd = 0., dsalpha_eta_dd = 0.,
           dsalpha_theta_j_dd = 0., dsalpha_theta_j_p1_dd = 0.,
           dsalpha_zeta_zp_dnu = 0., dsalpha_eta_dnu = 0.,
           dsalpha_theta_j_dnu = 0., dsalpha_theta_j_p1_dnu = 0.,
           dsalpha_zeta_zp_t_theta_j_dx = 0.,
           dsalpha_zeta_zp_t_theta_j_p1_dx = 0.,
           dsalpha_zeta_zp_r_t_theta_j_dx = 0.,
           dsalpha_zeta_zp_r_t_theta_j_p1_dx = 0.,
           dsalpha_eta_r_t_theta_j_dx = 0.,
           dsalpha_eta_r_t_theta_j_p1_dx = 0.,
           dsalpha_eta_t_theta_j_dx = 0.,
           dsalpha_eta_t_theta_j_p1_dx = 0.,
           dsalpha_eta_rdash_t_theta_j_dx = 0.,
           dsalpha_eta_rdash_t_theta_j_p1_dx = 0.,
           dsalpha_zeta_alpha = 0.;
    std::vector<std::complex<double>> dsalpha_zeta_zp_rp_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> dsalpha_eta_rp_dcs(m_n_rs, 0.);
    std::vector<std::complex<double>> dsalpha_eta_rpdash_dcs(m_n_rs, 0.);
    ...
    for (int k = 0; k < m_N_l; k++) {
      ...
      double zp_tks = m_omdd - rp_tks + 2. * d_rp_costkmnu;
      double zp_tk = std::sqrt(zp_tks);
      double zeta_alpha = this->zeta_power_term(zp_tk, m_power2_alpha);
      double eta = rp_tks - d_rp_costkmnu - d_drpdtheta_sintkmnu;
      salpha_planet += zeta_alpha * eta * m_l_weights[k];

      double dsalpha_dzeta = eta * m_l_weights[k];
      double dzeta_dzp = this->dzeta_power_term_dzp(zp_tk, m_power2_alpha);
      double dzeta_dalpha = this->dzeta_power_term_dalpha(
        zp_tk, m_power2_alpha
      );
      double dzp_dd = (rp_tk * costkmnu - d) / zp_tk;
      double dzp_dnu = d * rp_tk * sintkmnu / zp_tk;
      double dzp_drp = (d * costkmnu - rp_tk) / zp_tk;

      double dsalpha_deta = zeta_alpha * m_l_weights[k];
      double deta_dd = -rp_tk * costkmnu - drp_dtk * sintkmnu;
      double deta_dnu = -d * rp_tk * sintkmnu + d * drp_dtk * costkmnu;
      double deta_drp = 2. * rp_tk - d * costkmnu;
      double deta_drdash = -d * sintkmnu;

      dsalpha_zeta_zp_dd += dsalpha_dzeta * dzeta_dzp * dzp_dd;
      dsalpha_eta_dd += dsalpha_deta * deta_dd;

      dsalpha_zeta_zp_dnu += dsalpha_dzeta * dzeta_dzp * dzp_dnu;
      dsalpha_eta_dnu += dsalpha_deta * deta_dnu;
      dsalpha_zeta_alpha += dsalpha_dzeta * dzeta_dalpha;
      ...
      for (int n = -m_N_c; n < m_N_c + 1; n++) {
        ...
        std::complex<double> drpdash_dcn = (1. * n) * 1.i
                                           * std::exp((1. * n) * 1.i * t_k);
        dsalpha_zeta_zp_rp_dcs[npN_c] += dsalpha_dzeta * dzeta_dzp
                                         * dzp_drp * drp_dcn;
        dsalpha_eta_rp_dcs[npN_c] += dsalpha_deta * deta_drp * drp_dcn;
        dsalpha_eta_rpdash_dcs[npN_c] += dsalpha_deta * deta_drdash
                                         * drpdash_dcn;
      }
      ...
      if (theta_type_j == intersections::planet) {
        ...
        double dzp_dtk = -d * rp_tk * sintkmnu / zp_tk;
        double dsalpha_zeta_dzp = dsalpha_dzeta * dzeta_dzp;
        double dsalpha_zeta_zp_dtk = dsalpha_zeta_dzp * dzp_dtk;
        double dsalpha_zeta_zp_r_dtk = dsalpha_zeta_dzp * dzp_drp * drp_dtk;

        dsalpha_zeta_zp_t_theta_j_dx += dsalpha_zeta_zp_dtk * dtk_dtheta_j;
        dsalpha_zeta_zp_t_theta_j_p1_dx += dsalpha_zeta_zp_dtk * dtk_dtheta_j_p1;
        dsalpha_zeta_zp_r_t_theta_j_dx += dsalpha_zeta_zp_r_dtk * dtk_dtheta_j;
        dsalpha_zeta_zp_r_t_theta_j_p1_dx += dsalpha_zeta_zp_r_dtk * dtk_dtheta_j_p1;

        double drdash_dtk = this->d2rp_dtheta2(t_k);
        double deta_dtk = d * (rp_tk * sintkmnu - drp_dtk * costkmnu);
        double dsalpha_eta_dtk = dsalpha_deta * deta_dtk;
        double dsalpha_eta_rp_dtk = dsalpha_deta * deta_drp * drp_dtk;
        double dsalpha_eta_rdash_dtk = dsalpha_deta * deta_drdash * drdash_dtk;

        dsalpha_eta_t_theta_j_dx += dsalpha_eta_dtk * dtk_dtheta_j;
        dsalpha_eta_t_theta_j_p1_dx += dsalpha_eta_dtk * dtk_dtheta_j_p1;
        dsalpha_eta_r_t_theta_j_dx += dsalpha_eta_rp_dtk * dtk_dtheta_j;
        dsalpha_eta_r_t_theta_j_p1_dx += dsalpha_eta_rp_dtk * dtk_dtheta_j_p1;
        dsalpha_eta_rdash_t_theta_j_dx += dsalpha_eta_rdash_dtk * dtk_dtheta_j;
        dsalpha_eta_rdash_t_theta_j_p1_dx += dsalpha_eta_rdash_dtk * dtk_dtheta_j_p1;
      }
    }

    m_salpha += half_theta_range * salpha_planet;

    dsalpha_theta_j_dd += -fractions::one_half * salpha_planet * m_dthetas_dd[_j];
    dsalpha_theta_j_p1_dd += fractions::one_half * salpha_planet
                             * m_dthetas_dd[_j + 1];
    dsalpha_theta_j_dnu += -fractions::one_half * salpha_planet * m_dthetas_dnu[_j];
    dsalpha_theta_j_p1_dnu += fractions::one_half * salpha_planet
                              * m_dthetas_dnu[_j + 1];

    m_dsalpha_dd += dsalpha_theta_j_dd + dsalpha_theta_j_p1_dd
                    + half_theta_range * (
      dsalpha_zeta_zp_dd + dsalpha_eta_dd
      + m_dthetas_dd[_j] * (dsalpha_zeta_zp_t_theta_j_dx
                          + dsalpha_zeta_zp_r_t_theta_j_dx
                          + dsalpha_eta_t_theta_j_dx
                          + dsalpha_eta_r_t_theta_j_dx
                          + dsalpha_eta_rdash_t_theta_j_dx)
      + m_dthetas_dd[_j + 1] * (dsalpha_zeta_zp_t_theta_j_p1_dx
                              + dsalpha_zeta_zp_r_t_theta_j_p1_dx
                              + dsalpha_eta_t_theta_j_p1_dx
                              + dsalpha_eta_r_t_theta_j_p1_dx
                              + dsalpha_eta_rdash_t_theta_j_p1_dx));

    m_dsalpha_dnu += dsalpha_theta_j_dnu + dsalpha_theta_j_p1_dnu
                     + half_theta_range * (
      dsalpha_zeta_zp_dnu + dsalpha_eta_dnu
      + m_dthetas_dnu[_j] * (dsalpha_zeta_zp_t_theta_j_dx
                           + dsalpha_zeta_zp_r_t_theta_j_dx
                           + dsalpha_eta_t_theta_j_dx
                           + dsalpha_eta_r_t_theta_j_dx
                           + dsalpha_eta_rdash_t_theta_j_dx)
      + m_dthetas_dnu[_j + 1] * (dsalpha_zeta_zp_t_theta_j_p1_dx
                               + dsalpha_zeta_zp_r_t_theta_j_p1_dx
                               + dsalpha_eta_t_theta_j_p1_dx
                               + dsalpha_eta_r_t_theta_j_p1_dx
                               + dsalpha_eta_rdash_t_theta_j_p1_dx));

    m_dsalpha_dpower2_alpha += half_theta_range * dsalpha_zeta_alpha;

    for (int n = -m_N_c; n < m_N_c + 1; n++) {
      int npN_c = n + m_N_c;
      std::complex<double> dsalpha_theta_j_dcn =
        -fractions::one_half * salpha_planet * m_dthetas_dcs[npN_c][_j];
      std::complex<double> dsalpha_theta_j_p1_dcn =
        fractions::one_half * salpha_planet * m_dthetas_dcs[npN_c][_j + 1];

      m_dsalpha_dcs(npN_c) += dsalpha_theta_j_dcn + dsalpha_theta_j_p1_dcn
                              + half_theta_range * (
        dsalpha_zeta_zp_rp_dcs[npN_c] + dsalpha_eta_rp_dcs[npN_c]
        + dsalpha_eta_rpdash_dcs[npN_c]
        + m_dthetas_dcs[npN_c][_j] * (dsalpha_zeta_zp_t_theta_j_dx
                                    + dsalpha_zeta_zp_r_t_theta_j_dx
                                    + dsalpha_eta_t_theta_j_dx
                                    + dsalpha_eta_r_t_theta_j_dx
                                    + dsalpha_eta_rdash_t_theta_j_dx)
        + m_dthetas_dcs[npN_c][_j + 1] * (dsalpha_zeta_zp_t_theta_j_p1_dx
                                        + dsalpha_zeta_zp_r_t_theta_j_p1_dx
                                        + dsalpha_eta_t_theta_j_p1_dx
                                        + dsalpha_eta_r_t_theta_j_p1_dx
                                        + dsalpha_eta_rdash_t_theta_j_p1_dx));
