...
def _select_transit_eval_params(params, transit_engine, param_method):
    out = dict(params)
...
def build_harmonica_limb_dataframe(wavelengths, wavelength_err, rors_samples, harmonic_samples,
    ...
    eve_depth_samp = (r_evening_samp**2) * 1e6
    mor_depth_samp = (r_morning_samp**2) * 1e6
    a0_depth_samp = (a0_samp**2) * 1e6

    eve_med = np.nanpercentile(eve_depth_samp, 50, axis=0)
    eve_lo = eve_med - np.nanpercentile(eve_depth_samp, 16, axis=0)
    eve_hi = np.nanpercentile(eve_depth_samp, 84, axis=0) - eve_med

    mor_med = np.nanpercentile(mor_depth_samp, 50, axis=0)
    mor_lo = mor_med - np.nanpercentile(mor_depth_samp, 16, axis=0)
    mor_hi = np.nanpercentile(mor_depth_samp, 84, axis=0) - mor_med

    a0_depth_med = np.nanpercentile(a0_depth_samp, 50, axis=0)
    a0_depth_lo = a0_depth_med - np.nanpercentile(a0_depth_samp, 16, axis=0)
    a0_depth_hi = np.nanpercentile(a0_depth_samp, 84, axis=0) - a0_depth_med

    # Raw Rp/Rs statistics (needed for transmission string plots)
    a0_raw_med = np.nanpercentile(a0_samp, 50, axis=0)
    a0_raw_lo = a0_raw_med - np.nanpercentile(a0_samp, 16, axis=0)
    a0_raw_hi = np.nanpercentile(a0_samp, 84, axis=0) - a0_raw_med

    # eve_* came from a0+odd_sum (theta=0) = morning/leading limb
    # mor_* came from a0-odd_sum (theta=pi) = evening/trailing limb
    limb_df = pd.DataFrame({
        "wavelength": wl_arr,
        "wavelength_err": wl_err_arr,
        "depth_morning_median": eve_med,
        "depth_morning_err_lo": eve_lo,
        "depth_morning_err_hi": eve_hi,
        "depth_evening_median": mor_med,
        "depth_evening_err_lo": mor_lo,
        "depth_evening_err_hi": mor_hi,
...
def save_harmonica_limb_products(
    ...
    ax.errorbar(
        ...
        ecolor="red",
        capsize=0,
        label="Evening limb (trailing, $\\theta=\\pi$)",
    )
    ax.errorbar(
        wl_arr + offset,
        ...
        ecolor="blue",
        capsize=0,
        label="Morning limb (leading, $\\theta=0$)",
    )
    ax.set_xlabel("Wavelength [$\\mu$m]", fontsize=12)
    ax.set_ylabel("Transit Depth [ppm]", fontsize=12)
    ax.set_title(f"{title_prefix} - Evening/Morning Limb Spectra", fontsize=13)
...
def main():
    ...
    if analysis_stage not in {'all', 'prep', 'highres'}:
        raise ValueError(
            "analysis_stage must be one of {'all', 'prep', 'highres'}. "
    ...
    harmonica_spectro_parameterization = str(
        flags.get('harmonica_spectro_parameterization', 'delta_r')
    ).lower()
    if harmonica_spectro_parameterization not in {'fractional', 'delta_r'}:
        raise ValueError(
    ...
    if harmonica_spectro_odd_frac_sigma <= 0:
        raise ValueError("flags.harmonica_spectro_odd_frac_sigma must be > 0.")
    ...
    if transit_engine == 'harmonica':
        ...
        _engine_wl_kw = {'max_harmonic_order': max_harmonic_order, 'param_method': param_method}
        _engine_spectro_kw = {
        ...
        from models.jaxoplanet import create_whitelight_model, create_vectorized_model, NUTS_KWARGS, derive_geometry
        _engine_wl_kw = {'ld_profile': ld_profile, 'param_method': param_method}
        _engine_spectro_kw = {'ld_profile': ld_profile, 'param_method': param_method}
    ...
    # NOTE: The old C++ FFI custom calls were CPU-only, but harmonica now has
    # a pure JAX GPU path for N_c<=1 power-2 transits (power2_nc1_jax.py).
    # GPU is supported when harmonica_max_order <= 1 and ld_profile == 'power2'.
    if transit_engine == 'harmonica' and host_device != 'cpu':
        if max_harmonic_order > 1:
            ...
            print(
                f"Using pure JAX harmonica GPU path (N_c=1 power-2 LD)."
            )
    ...
    if not stringcheck or ('gp' in detrending_type):
        if not os.path.exists(f'{output_dir}/{instrument_full_str}_whitelight_GP_database.csv'):
            ...
            try:
                ...
                print(">>> Gradient diagnostic complete.\n")
            except Exception as e:
            ...
            if transit_engine == 'harmonica':
                ...
                print(f"NUTS settings: {NUTS_KWARGS}")
            mcmc = numpyro.infer.MCMC(
            ...
            if 'gp' in detrending_type:
                planet_model_masked = compute_transit_model_auto(model_eval_params_wl, t_masked)
            ...
            transit_only_model = compute_transit_model_auto(model_eval_params_wl, t_masked) + 1.0
            ...
            wl_model_vector = np.array(compute_transit_model_auto(model_eval_params_wl, jnp.array(wl_time_vector)) + 1.0)
    ...
    if need_lowres_analysis and valid is None:
        ...
        samples_lr = _run_sampling_stage(
            ...
            flux_lr,
            init_params_lr,
            nuts_kwargs=harmonica_lr_nuts_kwargs if transit_engine == 'harmonica' else NUTS_KWARGS,
            mcmc_kwargs=lowres_mcmc_kwargs,
            use_chunked=(spectro_sampling_mode == 'independent'),
            chunk_size=vmap_chunk_size,
            chunk_mode=chunk_mode,
            parallel_job_count=chunk_parallel_job_count,
            parallel_job_index=chunk_parallel_job_index,
            output_dir=output_dir,
            checkpoint_prefix=f"{instrument_full_str}_{lr_bin_str}",
            **model_run_args_lr,
        )
    ...
    samples_hr = _run_sampling_stage(
        ...
        flux_hr,
        init_params_hr,
        nuts_kwargs=harmonica_hr_nuts_kwargs if transit_engine == 'harmonica' else NUTS_KWARGS,
        mcmc_kwargs=highres_mcmc_kwargs,
        use_chunked=(spectro_sampling_mode == 'independent' or vmap_chunk_size is not None),
        chunk_size=vmap_chunk_size,
        chunk_mode=chunk_mode,
        parallel_job_count=chunk_parallel_job_count,
        parallel_job_index=chunk_parallel_job_index,
