...
def depth_err_H_from_err(err, teq, rp_re, mp_me, rstar_rs):
    ...
    return 1 / ((2 * (rp_re * R_E) * H*1e6) / (rstar_rs * R_S)**2)
...
plt.ylabel("PPM of CO2 Feature @ μ=20 amu (NIRSpec 4.0–4.3µm)")
plt.yscale("log")
plt.tight_layout()
plt.savefig("nirspec_neptune_ppm.png", dpi=300, bbox_inches='tight')
plt.show()
