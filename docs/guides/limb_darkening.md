...
`stellarprior` and `stellar` are aliases for `informed`. **Fixed.** Use
calculated coefficients without sampling them.
...
The fitter performs the validated gray-offset calibration automatically. See
...
version of the same prior. Wide-Gaussian and uniform/free power-2 priors retain
their existing behavior. Builders save `u1`, `u2`, `l`, `delta`, and `u` for
the quadratic path so downstream output remains in familiar quantities.
...
It computes an inverse-variance-weighted fitted-minus-model gray offset across
wavelength. The JSON artifact contains both offsets, uncertainties, channel
counts, and a fingerprint. This calibration is automatic for `ld_prior: sing`.
The code's tabulated fallback offsets are $\Delta l=0.020$ and
$\Delta\delta=-0.003$, with scatters 0.031 and 0.016. The log states when it
uses that fallback.
