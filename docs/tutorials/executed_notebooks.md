...
They include their saved output, so you can read the workflow without installing Koala or obtaining the original FITS files. To rerun one, set `KOALA_EXAMPLE_DATA_ROOT` to the directory containing the extracted FITS file and `KOALA_NOTEBOOK_RESULT_ROOT` to a new results directory. The SOSS notebook defaults to the compact file in `examples/data/`.
