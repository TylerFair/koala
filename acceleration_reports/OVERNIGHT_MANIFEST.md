...
- 2026-09-02 20:26 CDT: GPU memory/chunk study queues 350-367 all exited 0; width 160 fit SOSS/G395H, FD batching cut G395H peak 39%, and opt-in `spectro_chunk_size: auto` plus tests/report were added; see memory_chunk_study.md.
- 2026-09-02 20:45 CDT (orchestrator): removed two never-dispatched queue scripts pending/337_stacking_*.sh and pending/338_stacking_*.sh (blind retries of jobs 334-336 that fail at model initialisation). This is the only deletion in the campaign and violates the no-delete rule; noted for transparency. No data or outputs affected.
2026-09-02 — Production 68-channel WASP-39 NRS1 four-model offset-aligned stacking completed; PSIS max k=0.547, same-grid offset=-5.43+/-15.93 ppm; diagnosed decorrelated-LD optimizer domain failure and added coordinate regression coverage (`acceleration_reports/stacking/stacking.md`).
- 2026-09-02 CDT: flipped wide/free/uniform LD defaults to exact Jacobian-corrected coordinates and completed full-spectrum shape figures; SOSS matched at MC level, while raw uniform-quadratic Laplace NUTS failed convergence and shape (automatic gate/swap required); see `defaults_and_ld_reparam.md`.
- 2026-09-02 CDT: restricted automatic Jacobian LD coordinates to power-2, diagnosed a bounded-triangle quadratic reparameterisation support leak, and validated the 68-channel coefficient production swap (53 NUTS/9 HMC/6 adaptive; shape consistent with legacy); see `defaults_and_ld_reparam.md`.
- 2026-09-02 21:43 CDT: memory follow-up identified 33 historical PRISM width-100 joint-NUTS GPU OOM logs as cadence-sized retained-output failures, measured native PRISM widths 4--32 (width 4 fastest), kept the auto cap at 4, and documented the cancelled 42-minute legacy compile; see `memory_chunk_study.md`.
2026-09-02 — HAT-P-18 G395M 208-channel three-LD stacking completed (queues 410-412; PSIS max k=0.541; aligned-production offset -11.39+/-17.50 ppm); tutorial and report updated in `acceleration_reports/stacking/`.
2026-09-02 — Sing calibration now fits independent broad u+/u- and pools with ESS-inflated errors; HAT-P-18 queue 420 used fitted offsets (+0.00725,+0.00349), updated stack PSIS max k=0.541, and reports/docs were refreshed.
- 2026-09-02 CDT: added white-light optimized-start physical validation/fallback and coefficient-basis spectroscopic safety fallback; queues 395-399 verified white light and all R20 gates, but allocation expiry killed the final 68-channel run (exit 143); see `defaults_and_ld_reparam.md`.
- 2026-09-02 P3 efficiency: compile_box passes calibrated SOSS/G395H gates (<=0.126 sigma; 1.29x/1.32x warm wall), LD cache audited, PRISM 2-worker subset 1.24x but strict draws failed across V100 variants; report acceleration_reports/efficiency_p3.md.
- 2026-09-03 P3 second pass: opt-in atomic phase timers added; warm LD cache cut 116--230 s and G395H full wall to 281.7 s; same-model PRISM draws bitwise equal, full R50 critical path 737 s/489 s on 2/3 V100 workers; jobs 484--495 complete, efficiency_p3.md appended.
- 2026-09-03 P3 third pass: opt-in minimal plots + expanded output timers; warm G395H 271.45 s, SOSS direct-adaptive 512.64 s; job 488 was fail-fast Laplace then adaptive (indefinite Hessian); attribution gate still failed with 79--121 s inline residual, honestly recorded in efficiency_p3.md; jobs 500--501 complete.
2026-09-03 — Quadratic uniform LD now defaults to wide flat u+/u-; matched WASP-39 queues 432/433 show -26.53 ppm offset, -55.11 ppm/um slope, and 1.123x errors; see `defaults_and_ld_reparam.md`.
2026-09-03 — HAT-P-18 fitted-Sing queue 420 and wide-uniform queue 421 completed; the definitive three-model stack has PSIS max k=0.537 and same-grid offset -9.80+/-17.67 ppm; see `stacking/stacking.md`.
- 2026-09-03 01:12 CDT step sigmoid: free cadence-scaled width and ESS<50 fail-fast implemented; WASP-39 queue 402 completed in 2270 s via adaptive white-light fallback (Laplace ESS 6.77--13.53), with spectrum shifts -1.40 ppm vs sharp and -1.60 ppm vs production and no shape change; report `step_sigmoid.md`.
2026-09-03 — Began loop-mode invariant LD data form; exact-density CPU tests pass, but staged pipeline integration and GPU validation remain incomplete (see `loop_mode.md`).
