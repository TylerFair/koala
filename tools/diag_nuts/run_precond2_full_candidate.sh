...
case_name=${1:?usage: run_precond2_full_candidate.sh soss|g395h}
...
if [[ "$case_name" == soss ]]; then
  ...
  output=$output_dir/stellar_soss_high_fd_nuts_d5_full
  ...
  output=$output_dir/g395h_high_fd_nuts_d5_full
else
...
"$python_bin" tools/run_sampler_on_stage_inputs.py "$dump" \
  --backend independent_nuts --start 0 --end "$end" --warmup 1000 \
