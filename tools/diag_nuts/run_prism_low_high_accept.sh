...
"$python_bin" "$driver" "${common[@]}" \
  --backend independent_hmc --nuts-override num_steps=8 \
  --nuts-override trajectory_jitter=0.25 \
  --output-prefix "$output_dir/prism_low_map200_r5_hmc8_j25_ta99"

"$python_bin" "$driver" "${common[@]}" \
  --backend independent_nuts --nuts-override laplace_max_tree_depth=5 \
  --output-prefix "$output_dir/prism_low_map200_r5_nuts5_ta99"
