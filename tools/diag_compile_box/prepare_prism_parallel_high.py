...
dst=Path('/scratch/midway3/tfairnington/HAT-P-65_PRISM_P2_SPEED_PARALLEL_HIGH')
...
base['output_dir']='HAT-P-65_PRISM_P2_SPEED_PARALLEL_HIGH'; f=base['flags']; f.update(analysis_stage='highres',need_lowres=False,chunk_mode='parallel',chunk_parallel_job_count=2)
for i in (0,1):
 c=yaml.safe_load(yaml.safe_dump(base)); c['flags']['chunk_parallel_job_index']=i
 Path(f'configs_speed/HAT-P-65_PRISM_P2_SPEED_PARALLEL_HIGH_{i}.yaml').write_text(yaml.safe_dump(c,sort_keys=False))
