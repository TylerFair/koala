...
class TestFlux(unittest.TestCase):
    ...
    def test_api_power2_limb_darkening_matches_exact_shared_basis_terms(self):
        """Test exact power-2 LD against matching non-linear basis terms."""
        rs = np.array([0.1, 0.001, 0.001], dtype=np.float64)
        self._build_test_data_structures(n_dp=512, start=4.4, stop=5.6)
        c = 0.45
        expected_coeffs = {
        ...
        def build_flux(law, us):
            ht = HarmonicaTransit(self.times)
            ht.set_orbit(
                self.t0, self.period, self.a, self.inc,
                self.ecc_non_zero, self.omega
            )
            ht.set_stellar_limb_darkening(np.asarray(us, dtype=np.float64), law)
            ht.set_planet_transmission_string(rs)
            return ht.get_transit_light_curve()

        flux_power2_dash = build_flux('power-2', [c, 1.0])
        flux_power2_plain = build_flux('power2', [c, 1.0])
        np.testing.assert_allclose(
            flux_power2_dash, flux_power2_plain, rtol=1.e-12, atol=1.e-12
        )

        for alpha, nonlinear_coeffs in expected_coeffs.items():
            flux_power2 = build_flux('power-2', [c, alpha])
            flux_nonlinear = build_flux('non-linear', nonlinear_coeffs)
            np.testing.assert_allclose(
                flux_power2, flux_nonlinear, rtol=1.e-12, atol=1.e-12
            )
